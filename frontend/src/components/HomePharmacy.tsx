...
<>
    <div className="healthadvice-card">
        ...
        <p className="healthadvice-paragraph">Ett litet apotek för ditt djur bör innehålla följande</p>
        <ul className="healthadvice-list">
            {homePharmacyItems.map((item, index) => (
                <li key={index} className="healthadivce-li">{item}</li>
            ))}
        </ul>
        <p className="healthadvice-paragraph-italic">Tips! De flesta djurbutiker och veterinärkliniker säljer första hjälpen kit för husdjur</p>
    </div>
